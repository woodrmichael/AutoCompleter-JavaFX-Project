...
public class BenchmarkerFX extends Application {
    ...
    public void start(Stage stage) {
        final String help = AutoCompleterBenchmarker.getHelp();
        final Alert alert = new Alert(Alert.AlertType.ERROR);
        alert.setWidth(WIDTH);
        alert.setHeight(HEIGHT);
        try {
            final Map<String, String> arguments = getParameters().getNamed();
            final int argsCount = 7;
            if(arguments.size() != argsCount) {
                throw new IllegalArgumentException("Please ensure all arguments are present");
            }

            final String implementation = arguments.get("implementation");
            final String backingListType = arguments.get("backingListType");
            final String operation = arguments.get("operation");
            final int startSize = Integer.parseInt(arguments.get("startSize"));
            final int multiplier = Integer.parseInt(arguments.get("multiplier"));
            final int numberOfSamples = Integer.parseInt(arguments.get("numberOfSamples"));
     ...
     *              on the AutoCompleter.
     * @param startSize the original size of the backing list of the AutoCompleter.
     * @param multiplier how much the size of the backing list of the AutoCompleter
     * should multiply by between each test.
    ...
    private static void saveScreenShot(String filename, Scene scene) throws IOException {
        ...
        File file = new File(
                filename == null ? OUTPUT_FOLDER + "plot.png" : OUTPUT_FOLDER + filename);
